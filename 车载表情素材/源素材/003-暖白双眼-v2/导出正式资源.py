"""Export the approved soft-eyes animation into GIF/EAF delivery files."""
...
PACKAGE = ASSET_ROOT / "003-暖白双眼-v2"
GIF_DIR = PACKAGE / "gif"
EAF_DIR = PACKAGE / "eaf"
...
def quantize_eaf_frames(frames):
    ...
    return [frame.convert("RGB").quantize(palette=palette, dither=Image.Dither.NONE) for frame in frames]


def save_gif(path: Path, frames):
    palette_source = Image.new("RGB", (360, 360 * len(frames)), (0, 0, 0))
    for index, frame in enumerate(frames):
        palette_source.paste(frame.convert("RGB"), (0, index * 360))
...
def main() -> None:
    renderer = load_module(PREVIEW_RENDERER, "soft_eyes_preview_renderer")
    eaf_tool = load_module(EAF_TOOL_PATH, "soft_eyes_eaf_tool")
    ...
    for state in STATE_ORDER:
        frames = [renderer.render(state, index * FRAME_DURATION_MS / 1000) for index in range(FRAME_COUNT)]
        ...
        with Image.open(gif_path) as gif:
            gif_durations = [int(frame.info.get("duration", FRAME_DURATION_MS)) for frame in ImageSequence.Iterator(gif)]
            if gif.size != (360, 360) or len(gif_durations) < 2 or sum(gif_durations) != FRAME_COUNT * FRAME_DURATION_MS:
                raise ValueError(f"GIF 尺寸或帧数不符合要求：{state}")
    ...
    (EAF_DIR / "manifest.json").write_text(json.dumps(records, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    print(f"eaf_bytes={total}")
    print(f"source_sha256={hashlib.sha256((HERE / 'v2设计图.png').read_bytes()).hexdigest()}")
